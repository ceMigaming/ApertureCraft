...
public class RadioRenderer extends GeoEntityRenderer<RadioEntity> {
    ...
    void renderText(Vec3d pos, float entityYaw, MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
        matrices.push();
        Quaternionf q = new Quaternionf().rotateLocalY((float)(Math.PI-Math.toRadians(entityYaw)));
        matrices.multiply(q);
        matrices.scale(-0.005f, -0.005f, 0.005f);
        matrices.translate(-40, -36.0f, 2.0f);
        textRenderer.draw("85.2 FM", (float) textRenderer.getWidth("85.2 FM") / 2.0f, 0.0f, 0x27a7d8,
                false, matrices.peek().getPositionMatrix(), vertexConsumers, TextLayerType.NORMAL, 0, 0xF000F0);
